...
final private[dilate] case class DilateResult(
...
final private[dilate] case class OwnerClassArgsSplitted(
  nonImplicitArgs: Seq[Term.Param] = Seq.empty,
  implicitArgs: Seq[Term.Param] = Seq.empty
)
final private[dilate] case class Extraction(
  newArgs: Term.Param,
  valueclass: Option[Defn.Class],
  implicitDef: Option[Defn.Def]
)
...
object Dilate {
  ...
  private[dilate] def extract(
    ...
    val companionObjectTemplate = CompanionObjectTemplate(
      ...
      implicitDefs = extraction.flatMap(_.implicitDef)
    )

    DilateResult(template = companionObjectTemplate, domain = newArgs)
  }
  ...
  private[this] def returnExtraction(
    ownerClassName: Type.Name,
    path: String
  )(implicit param: Term.Param) =
    Extraction(
  ...
  private[this] def buildOwnerClassArgs(
    typeName: Type.Name
  )(implicit param: Term.Param): Term.Param =
    Term.Param(
      mods = param.mods,
  ...
  /* build value class */
  private[this] def buildValueClass(
    argTypeName: Type.Name
  )(implicit param: Term.Param): Option[Defn.Class] = {

    val valueClassTerm: Term.Param = Term.Param.apply(
      mods = Seq.empty,
      name = Term.Name("value"),
      decltpe = Option.apply(argTypeName),
      default = None
    )
    Try(q"""${Mod.Case.apply()} class ${
      Type
        .Name(param.name.value.capitalize)
    }($valueClassTerm) extends AnyVal""").toOption
  }

  private[this] def buildImplicitDef(
    typeName: Type.Name
  )(implicit param: Term.Param): Option[Defn.Def] =
    Try(
      Defn.Def.apply(
        mods = Seq(Mod.Implicit.apply()),
        name = Term.Name(s"to${param.name.value.capitalize}"),
        tparams = Nil,
        paramss = Seq(
          Seq(
            Term.Param.apply(
        ...
        decltpe = Option.apply(Type.Name(s"$typeName.${param.name.value.capitalize}")),
        body = q"""${Ctor.Ref.Name(s"$typeName.${param.name.value.capitalize}")}(${
          Term.Name(
            param.name.value
          )
        })"""
